...
public class MainActivity2 extends AppCompatActivity {
    ...
    EditText otp;
    Button Login;
    String receivedOTP;   // class-level variable
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ✅ assign to class variable (NO "String" here)
        receivedOTP = getIntent().getStringExtra("otp");
        String receivedEmail = getIntent().getStringExtra("email"); // Get email
        ...
        Login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (enteredOTP.equals(receivedOTP)) {
                    ...
                    editor.putString("patient_email", receivedEmail);
                    editor.apply();

                    // ✅ Only go to Dashboard if OTP is correct
                    Intent intent = new Intent(MainActivity2.this, Dashboard.class);
                    startActivity(intent);
                    finish();
                } else {
        ...
        // -------- Translation part (kept similar to your style) ----------
        SharedPreferences prefs = getSharedPreferences("settings", MODE_PRIVATE);
        String targetLang = prefs.getString("language", "en");
        ...
        translator.downloadModelIfNeeded(conditions)
                .addOnSuccessListener(unused -> {
                    // Usually we translate static text (button text), not the user's OTP input
                    translator.translate(Login.getText().toString())
                            .addOnSuccessListener(translated -> Login.setText(translated));
                })
                .addOnFailureListener(e -> {
